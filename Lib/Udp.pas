...
type
  ...
  protected
    ...
    function SocketWriteBuffer(Socket: TSocket; Buffer: Pointer; Size, Flag:
      integer): integer;
    function SocketReadBuffer(Socket: TSocket; Buffer: Pointer; Size, Flag:
      integer): integer;

    procedure SocketWriteTo(Socket: TSocket; Flag: integer; Data: string; var
      SockAddrIn: TSockAddrIn);
    function SocketReadFrom(Socket: TSocket; Flag: integer; var SockAddrIn:
      TSockAddrIn): string;
    function SocketWriteBufferTo(Socket: TSocket; Buffer: Pointer; Size, Flag:
      integer; var SockAddrIn: TSockAddrIn): integer;
    function SocketReadBufferFrom(Socket: TSocket; Buffer: Pointer; Size, Flag:
      integer; var SockAddrIn: TSockAddrIn): integer;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    {Address and Port Resolving Helper Functions}
    function GetSockAddrIn(Host, Port: string; var SockAddrIn: TSockAddrIn):
      boolean;
    function GetAnySockAddrIn(Port: string; var SockAddrIn: TSockAddrIn):
      boolean;
    function GetBroadcastSockAddrIn(Port: string; var SockAddrIn: TSockAddrIn):
      boolean;
  ...
  public
    ...
    function WriteBuffer(Socket: TSocket; Buffer: Pointer; Size: integer):
      integer;
    function ReadBuffer(Socket: TSocket; Buffer: Pointer; Size: integer):
      integer;
  ...
  public
    ...
    function WriteBuffer(Socket: TSocket; Buffer: Pointer; Size: integer; var
      SockAddrIn: TSockAddrIn): integer;
    function ReadBuffer(Socket: TSocket; Buffer: Pointer; Size: integer; var
      SockAddrIn: TSockAddrIn): integer;
...
procedure Register;
begin
  RegisterComponents('Samples', [TTCPClient, TTCPServer, TUDPClient,
    TUDPServer]);
end;
...
function TCusSok.SocketWriteBuffer(Socket: TSocket; Buffer: Pointer; Size, Flag:
  integer): integer;
begin
...
function TCusSok.SocketReadBuffer(Socket: TSocket; Buffer: Pointer; Size, Flag:
  integer): integer;
begin
...
procedure TCusSok.SocketWriteTo(Socket: TSocket; Flag: integer; Data: string; var
  SockAddrIn: TSockAddrIn);
var
  TotSent, ToSend, Sent, ErrorLoop: integer;
begin
  ...
  begin
    ...
    ToSend := Length(Data);
    repeat
      Sent := sendto(Socket, Data[TotSent + 1], (ToSend - TotSent), Flag,
        SockAddrIn, SizeOf(SockAddrIn));
      if Sent = SOCKET_ERROR then
      begin
        Inc(ErrorLoop);
        if WSAGetLastError <> WSAEWOULDBLOCK then
...
function TCusSok.SocketReadFrom(Socket: TSocket; Flag: integer; var SockAddrIn:
  TSockAddrIn): string;
var
  Len: integer;
  Received: longint;
begin
  Len := SizeOf(SockAddrIn);
  Received := recvfrom(Socket, FReadBuffer, SizeOf(TReadBuffer), Flag,
    SockAddrIn, Len);
  if Received = SOCKET_ERROR then
  begin
    if WSAGetLastError <> WSAEWOULDBLOCK then
...
function TCusSok.SocketWriteBufferTo(Socket: TSocket; Buffer: Pointer; Size,
  Flag: integer; var SockAddrIn: TSockAddrIn): integer;
begin
...
function TCusSok.SocketReadBufferFrom(Socket: TSocket; Buffer: Pointer; Size,
  Flag: integer; var SockAddrIn: TSockAddrIn): integer;
var
  Len: integer;
begin
...
begin
  ...
  begin
    ...
    if WSAASyncSelect(FLocalSocket, FHandle, WM_ASYNCSELECT, EventMask) <> 0
      then
      SocketError(WSAGetLastError)
    else
    begin
...
begin
  ...
  if setsockopt(FLocalSocket, SOL_SOCKET, SO_OOBINLINE, PChar(@SockOpt),
    SizeOf(SockOpt)) <> 0 then
  begin
    SocketError(WSAGetLastError);
    closesocket(FLocalSocket);
    Exit;
...
{$ENDIF}
    ...
    SockOpt := true; {Enable OOB Data inline}
    if setsockopt(NewSocket, SOL_SOCKET, SO_OOBINLINE, PChar(@SockOpt),
      SizeOf(SockOpt)) <> 0 then
    begin
      SocketError(WSAGetLastError);
      closesocket(NewSocket);
...
function TTCPServer.WriteBuffer(Socket: TSocket; Buffer: Pointer; Size:
  integer): integer;
begin
  Result := SocketWriteBuffer(Socket, Buffer, Size, 0);
end;

function TTCPServer.ReadBuffer(Socket: TSocket; Buffer: Pointer; Size: integer):
  integer;
begin
...
begin
  ...
  if setsockopt(FLocalSocket, SOL_SOCKET, SO_BROADCAST, PChar(@SockOpt),
    SizeOf(SockOpt)) <> 0 then
  begin
    SocketError(WSAGetLastError);
    closesocket(FLocalSocket);
...
procedure TUDPServer.Write(Socket: TSocket; Data: string; var SockAddrIn:
  TSockAddrIn);
begin
...
function TUDPServer.WriteBuffer(Socket: TSocket; Buffer: Pointer; Size: integer;
  var SockAddrIn: TSockAddrIn): integer;
begin
  Result := SocketWriteBufferTo(Socket, Buffer, Size, 0, SockAddrIn);
end;

function TUDPServer.ReadBuffer(Socket: TSocket; Buffer: Pointer; Size: integer;
  var SockAddrIn: TSockAddrIn): integer;
begin
