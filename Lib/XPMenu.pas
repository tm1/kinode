...
begin
  ...
  begin
    ...
    {$IFDEF VER5U}
    if ((Comp is TCustomFrame) and (xccFrame in FXPContainers)) then
...
begin
  ...
  for y := 0 to ABitmap.Height - 1 do
    ...
    begin
      Color := ABitmap.Canvas.Pixels[x, y];
      if Color = LastColor1 then
        B.Canvas.Pixels[x, y] := LastColor2
      else
      begin
        LastColor2 := NewColor(B.Canvas, Color, Value);
        B.Canvas.Pixels[x, y] := LastColor2;
        LastColor1 := Color;
      end;
    end;

  ABitmap.Canvas.Draw(0, 0, B);
  B.Free;
...
begin
  ...
  try
    ...
    B2.Canvas.FillRect(Rect(0, 0, BWidth, BHeight));
    TransparentColor := B.Canvas.Pixels[0, B.Height - 1];
